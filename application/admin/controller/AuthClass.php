...
//            }
        ...
        foreach ($authList as $auth) {
            ...
            if ($mode == 'url' && $query == $auth) {
                ...
                if (in_array($auth, $name)) {  //如果节点相符且url参数满足
                    $list[] = $auth;
                }
    ...
    public function getGroups($uid) {
        ...
        $user_groups = db($this->config['auth_group_access'])->join($this->config['auth_group'], "{$this->config['auth_group_access']}.group_id={$this->config['auth_group']}.id")
                        ->where(['admin_id' => $uid])->select();
    ...
    protected function getAuthList($uid, $type) {
        ...
        foreach ($groups as $g) {
            $ids = array_merge($ids, explode(',', trim($g['type'], ',')));
        }
        ...
        //读取用户组所有权限规则
        $rules = Db::name($this->config['auth_rule'])->where($map)->field('type')->select();
        //循环规则，判断结果。
        $authList = [];   //
        foreach ($rules as $rule) {
            if (!empty($rule['type'])) { //根据condition进行验证
                $this->getUserInfo($uid); //获取用户信息,一维数组
                // $command = preg_replace('/\{(\w*?)\}/', '$user[\'\\1\']', $rule['type']);
                // @(eval('$condition=(' . $command . ');'));
                // $condition && 
                $authList[] = strtolower($rule['type']);

            } else {
                $authList[] = strtolower($rule['type']); //只要存在就记录
